...
pub fn handle_menu_event(app: &AppHandle, event: SystemTrayEvent) {
    match event {
        ...
        SystemTrayEvent::MenuItemClick { id, .. } => match id.as_str() {
            ...
            "issues" => {
								tauri::api::shell::open(&app.shell_scope(), "https://github.com/ddki/tauri-vite-app-demo/issues", None).unwrap();
            }
            "github" => {
								tauri::api::shell::open(&app.shell_scope(), "https://github.com/ddki/tauri-vite-app-demo", None).unwrap();
            }
            "check_update" => {
