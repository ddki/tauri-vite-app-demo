...
pub fn handle_menu_event(event: WindowMenuEvent) {
    match event.menu_item_id() {
        ...
        "issues" => {
						tauri::api::shell::open(&event.window().shell_scope(), "https://github.com/ddki/tauri-vite-app-demo/issues", None).unwrap();
        }
        "github" => {
						tauri::api::shell::open(&event.window().shell_scope(), "https://github.com/ddki/tauri-vite-app-demo", None).unwrap();
        }
        "check_update" => {
        ...
        "call_font" => event
            ...
            .unwrap(),
        "dialog_1" => {
						tauri::api::dialog::message(Some(event.window()), "Dialog Title", "我是弹窗-1");
        }
        _ => {}
    }
}
